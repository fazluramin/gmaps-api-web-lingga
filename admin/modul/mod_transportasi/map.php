...
include "config/koneksi.php";
    $sql="SELECT * FROM tbl_transportasi where nama_transp='$t[nama_transp]'";
...
The following list shows the approximate level of detail you can expect to see at each zoom level:
  ...
  if($num>0){
    ...
    while($row = mysql_fetch_array($query)){
      echo '['.$row['latitude'].','.$row['longitude'].',"'.$row['nama_transp'].'","'.$row['kategori'].'","'.$row['alamat'].'","'.$row['latitude'].'","'.$row['longitude'].'","'.$row['foto'].'"]';
      if($i<$num-1) 
        echo ','."\r\n";
      else 
        echo "\r\n";
...
?>
    ...
    for (i = 0; i < locations.length; i++) {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          infowindow.setContent("Tempat Ibadah : "+locations[i][2]+"<br />Kategori : "+locations[i][3]+"<br />Alamat : "+locations[i][4]+"<br />Latitude : "+locations[i][5]+"<br />Longitude : "+locations[i][6]+"<br /><br /><img src='img/gallery/transportasi/"+locations[i][7]+"' width='250' height='150'>\n");
