...
include "config/koneksi.php";
    $sql="SELECT * FROM tbl_hotel where nama_hotel='$t[nama_hotel]'";
...
The following list shows the approximate level of detail you can expect to see at each zoom level:
  ...
  if($num>0){
    ...
    while($row = mysql_fetch_array($query)){
      echo '['.$row['latitude'].','.$row['longitude'].',"'.$row['nama_hotel'].'","'.$row['telepon'].'","'.$row['alamat'].'","'.$row['latitude'].'","'.$row['longitude'].'","'.$row['foto'].'"]';
      if($i<$num-1) 
        echo ','."\r\n";
      else 
        echo "\r\n";
...
?>
    ...
    for (i = 0; i < locations.length; i++) {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          infowindow.setContent("Nama Hotel : "+locations[i][2]+"<br />Telepon : "+locations[i][3]+"<br />Alamat : "+locations[i][4]+"<br />Latitude : "+locations[i][5]+"<br />Longitude : "+locations[i][6]+"<br /><br /><img src='img/gallery/hotel/"+locations[i][7]+"' width='250' height='150'>\n");
