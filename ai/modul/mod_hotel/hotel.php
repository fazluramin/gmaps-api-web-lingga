...
switch($_GET[act]){
  ...
  <div>
        <ul class='breadcrumb'>
          ...
          <li>
            <li class='active'><i class='icon-map-marker'></i><font color='grey'> Hotel & Penginapan</font></a> <span class='divider'></span>
          </li>
        </ul></font>
      </div>
...
<div class='row-fluid sortable'>    
        <div class='box span12'>
          <div class='box-header well' data-original-title>
            <h2><i class='icon-map-marker'></i><font color='green'> Hotel & Penginapan</font></h2>&nbsp;|&nbsp;
            <a href='?module=hotel&act=input' type=button class='btn btn-success'><i class='icon-plus-sign icon-white'></i> Add Data</br></a>
            <div class='box-icon'>
          ...
          <th width='18%'><center><i class='icon-map-marker'></i> Hotel & Penginapan</th>
          <th width='5%'><center><i class='icon-picture'></i> Foto</th>
          <th width='17%'><center><i class='icon-info-sign'></i> Alamat</th>         
          <th width='20%'><center><i class='icon-map-marker'></i> Titik Lokasi</th>         
          <th width='23%'><center><i class='icon-wrench'></i> Action</th>     
                </tr>
              </thead>   
...
"; 
    ...
    while ($r=mysql_fetch_array($tampil)){
       echo "
            <td><center>".$no."</td>
            <td><center>$r[nama_hotel]</b></font></td>
            <td><center><li class='thumbnail'>
    <a title='$r[foto]' href='img/gallery/hotel/$r[foto]'><img src='img/gallery/hotel/$r[foto]' width='140' height='75'></td>
            <td>$r[alamat]
            </td>
        ...
        echo"
                <td class='center'><center>
                  <a href='?module=dt_hotel&id=$r[id_hotel]' class='btn btn-success' href='#'>
                    <i class='icon-zoom-in icon-white'></i>  
                    Detail                                            
                  </a>
                 <a href='?module=hotel&act=update&id=$r[id_hotel]' onclick=\"return confirm('Apakah anda yakin ingin mengubah data $r[nama_hotel]?')\" class='btn btn-warning' href='#'>
                    <i class='icon-edit icon-white'></i>  
                    Edit                                            
                  </a>
                  <a href=$aksi?module=hotel&act=hapus&id=$r[id_hotel] onclick=\"return confirm('Apakah anda yakin ingin menghapus data $r[nama_hotel]?')\" class='btn btn-danger' href='#'>
...
$tampil=mysql_fetch_array(mysql_query("select * from tbl_hotel where id_hotel=$_GET[id]"));
}
      ...
      <div class='row-fluid sortable'>    
        <div class='box span12'>
          <div class='box-header well' data-original-title>
            ...
            <font color='green'> &nbsp;Edit Data Hotel : $t[nama_hotel]</font></h2>            
            <div class='box-icon'>
              <a href='#' class='btn btn-minimize btn-round'><i class='icon-chevron-up'></i></a>
              <a href='#' class='btn btn-close btn-round'><i class='icon-remove'></i></a>
            </div>
...
<form method=POST enctype='multipart/form-data' action=$aksi?module=hotel&act=update>
<input type=hidden name=id value=$t[id_hotel]>
    ...
    <tr><td width='12%'><label class='control-label'>Nama Penginapan</td><td width='23%'><input type='text' name='nama_hotel' value='$t[nama_hotel]' style='width: 220px;' placeholder='Nama hotel'></td>
    
    <td width='140' height='400' rowspan='8' colspan='2' style='text-align: left'><center>
    ...
    <tr><td>Telepon</td><td><input type='text' name='telepon' value='$t[telepon]' style='width: 220px;' placeholder='Nomor Telepon'></td></tr>
    <tr><td>Alamat</td><td><textarea type='text' name='alamat' style='width: 220px; height: 130px;' placeholder='Alamat Lokasi  hotel'>$t[alamat]</textarea></td></tr>
    <tr><td>Fasilitas</td><td>
    

";?>
<select data-placeholder="Pilih Fasilitas Yang Tersedia" name="fasilitas[]"  multiple data-rel="chosen"
            style="width: 230px; height: 120px;"> 
<?php
      for ($a=0;$a<count($fasilitas);$a++)
    {  if(!empty($_GET['id']))
    {  if(is_fasilitas_selected($a,$_GET['id'])==1)
    {  echo '<option value="'.$a.'"/ selected>'.$fasilitas[$a].'
                 '; }

    else 

    {  echo '<option value="'.$a.'"/>'.$fasilitas[$a].''; }
...
<?php echo"
    ...
    <tr><td>Jumlah Kamar</td><td>
        <div class='input-append'>
        <input type='text' name='jml_kamar' value='$t[jml_kamar]' style='text-align: right; width: 170px;' placeholder='Jumlah Kamar Hotel'><span class='add-on'>Kamar</span>
        </div>
    </td></tr>

    <tr><td>Max Penginap</td><td>
        <div class='input-append'>
        <input type='text' name='jml_penginap' value='$t[jml_penginap]' style='text-align: right; width: 170px;' placeholder='Jumlah Maksimal Penginap'><span class='add-on'>Orang</span>
        </div>
    </td></tr>

    <tr><td>Harga</td><td>
        <div class='input-append'>
        <span class='add-on'>Rp.</span><input type='text' name='harga' value='$t[harga]' style='width: 188px;' placeholder='Harga Kamar Permalam'>
        </div>
    </td></tr>
...
//ketika link hapus di klik
      ...
      <div>
        <ul class='breadcrumb'>
         <li>
          ...
          <li>
            <a href='index.php?module=hotel' style='text-decoration:none'><li class='active'><i class='icon-map-marker'></i><font color='black'>  Hotel & Penginapan</font></a> <span class='divider'>/</span>
          </li>
          <li>
            <a style='text-decoration:none'><i class='icon-plus'></i><font color='grey'> Tambah  Hotel</font></a> <span class='divider'></span>
          </li>
        </ul>
      </div>
      <div class='row-fluid sortable'>    
        <div class='box span12'>
          <div class='box-header well' data-original-title>
            ...
            <h2>
            <a href='index.php?module=hotel' type=button class='btn btn btn-danger'><i class='icon icon-white icon-arrowreturn-se'></i></a> &nbsp;
            <i class='icon-map-marker'></i><font color='green'> Tambah Hotel</font></h2>            
            <div class='box-icon'>
              <a href='#' class='btn btn-minimize btn-round'><i class='icon-chevron-up'></i></a>
              <a href='#' class='btn btn-close btn-round'><i class='icon-remove'></i></a>
            </div>
  ...
  <form method='POST' enctype='multipart/form-data' action=$aksi?module=hotel&act=input>
  <table  class='table table-striped table-bordered bootstrap-datatable ' width='100%' border='0' align='center' cellpadding='10' cellspacing='10'>
    <tr><td width='12%'><label class='control-label'>Nama Penginapan</td><td width='23%'>
    <input type='text' name='nama_hotel' value='$t[nama_hotel]' style='width: 220px;' placeholder='Nama Hotel / Penginapan'>
    </td>
...
<?php
      for ($a=0;$a<count($fasilitas);$a++)
    {  if(!empty($_GET['id_hotel']))
    {  if(is_fasilitas_selected($a,$_GET['id_hotel'])==1)
    {  echo '<option value="'.$a.'"/ selected>'.$fasilitas[$a].'
                 '; }
...
<?php
    ...
    {  if(!empty($_GET['id_hotel']))
    {  if(is_fasilitas_selected($a,$_GET['id_hotel'])==1)
    {  echo '<input type="checkbox" name="fasilitas[]" value="'.$a.'" checked="checked"/>'.$fasilitas[$a].'<br/>'; }

    else 

    {  echo '<input type="checkbox" name="fasilitas[]" value="'.$a.'"/>'.$fasilitas[$a].'<br/>'; }
