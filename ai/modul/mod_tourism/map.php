...
include "koneksi.php";
    ...
    $sql="SELECT * FROM tbl_owisata where nama_owisata='$t[nama_owisata]'";
...
?>
  ...
  <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCQ8kwzLQgc7iFf7zC7PAaMizkoxcD_5bQ&callback=initMap"
  async defer></script>
    <style type="text/css">
...
The following list shows the approximate level of detail you can expect to see at each zoom level:
  ...
  if($num>0){
    ...
    while($row = mysql_fetch_array($query)){
      echo '['.$row['latitude'].','.$row['longitude'].',"'.$row['nama_owisata'].'","'.$row['kategori'].'","'.$row['alamat'].'","'.$row['latitude'].'","'.$row['longitude'].'","'.$row['foto'].'"]';
      if($i<$num-1) 
        echo ','."\r\n";
      else 
        echo "\r\n";
...
?>
    ...
    for (i = 0; i < locations.length; i++) {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          infowindow.setContent("Objek Wisata : "+locations[i][2]+"<br />Kategori : "+locations[i][3]+"<br />Alamat : "+locations[i][4]+"<br />Latitude : "+locations[i][5]+"<br />Longitude : "+locations[i][6]+"<br /><br /><img src='img/gallery/wisata/"+locations[i][7]+"' width='250' height='150'>\n");
